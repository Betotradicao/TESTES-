...
export default function VisionPalavraChave2() {
  ...
  const [configCameras, setConfigCameras] = useState([]);
  ...
  const openConfig = () => {
    setConfigCameras(camerasPdv.length > 0 ? [...camerasPdv] : [{ pdv: 1, channel: 0, label: 'Caixa 1', antes: 20, depois: 120 }]);
    setShowConfig(true);
  };

  const addConfigRow = () => {
    setConfigCameras(prev => [...prev, { pdv: prev.length + 1, channel: 0, label: '', antes: 20, depois: 120 }]);
  };

  const removeConfigRow = (idx) => {
    setConfigCameras(prev => prev.filter((_, i) => i !== idx));
  };

  const updateConfigRow = (idx, field, value) => {
    setConfigCameras(prev => {
      const copy = [...prev];
      copy[idx] = { ...copy[idx], [field]: field === 'label' ? value : Number(value) };
      return copy;
    });
  };
  ...
  const saveConfig = async () => {
    ...
    try {
      await api.post('/dvr-cftv/config/cameras-pdv', { cameras: configCameras });
      setCamerasPdv(configCameras);
      setShowConfig(false);
    } catch (err) {
  ...
  return (
    <Layout>
      {/* Modal Config Cameras PDV */}
      {showConfig && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={() => setShowConfig(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="bg-gradient-to-r from-purple-600 to-purple-500 px-6 py-4">
              <h3 className="text-white font-bold text-lg">Configurar Cameras por PDV</h3>
              <p className="text-purple-200 text-sm">Associe cada PDV a uma camera do DVR</p>
            </div>
            <div className="p-4 max-h-[60vh] overflow-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b">
                    ...
                    <th className="px-2 py-2 text-left">Canal DVR</th>
                    <th className="px-2 py-2 text-left">Nome</th>
                    <th className="px-2 py-2 text-left">Antes (s)</th>
                    <th className="px-2 py-2 text-left">Depois (s)</th>
                    <th className="px-2 py-2 w-10"></th>
                  </tr>
                </thead>
                <tbody>
                  {configCameras.map((cam, idx) => (
                    <tr key={idx} className="border-b">
                      <td className="px-2 py-1">
                        <input type="number" value={cam.pdv} onChange={e => updateConfigRow(idx, 'pdv', e.target.value)}
                          className="w-16 px-2 py-1 border rounded text-sm" />
                      </td>
                      <td className="px-2 py-1">
                        <input type="number" value={cam.channel} onChange={e => updateConfigRow(idx, 'channel', e.target.value)}
                          className="w-16 px-2 py-1 border rounded text-sm" />
                      </td>
                      <td className="px-2 py-1">
                        <input type="text" value={cam.label} onChange={e => updateConfigRow(idx, 'label', e.target.value)}
                          className="w-full px-2 py-1 border rounded text-sm" placeholder="Ex: Caixa 1" />
                      </td>
                      <td className="px-2 py-1">
                        <input type="number" value={cam.antes} onChange={e => updateConfigRow(idx, 'antes', e.target.value)}
                          className="w-16 px-2 py-1 border rounded text-sm" />
                      </td>
                      <td className="px-2 py-1">
                        <input type="number" value={cam.depois} onChange={e => updateConfigRow(idx, 'depois', e.target.value)}
                          className="w-16 px-2 py-1 border rounded text-sm" />
                      </td>
                      <td className="px-2 py-1">
                        <button onClick={() => removeConfigRow(idx)} className="text-red-500 hover:text-red-700">
              ...
              <button onClick={addConfigRow} className="mt-2 px-3 py-1 text-sm text-purple-600 hover:bg-purple-50 rounded">
                + Adicionar PDV
              </button>
            </div>
            <div className="px-4 py-3 bg-gray-50 border-t flex justify-end gap-2">
              <button onClick={() => setShowConfig(false)} className="px-4 py-2 text-sm text-gray-600 hover:bg-gray-200 rounded-lg">Cancelar</button>
        ...
        <div className="bg-gradient-to-br from-purple-600 to-indigo-700 rounded-lg shadow-lg p-4 mb-3 text-white">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button onClick={openConfig} className="bg-white/20 hover:bg-white/30 backdrop-blur-sm rounded-lg px-3 py-2 text-sm font-medium transition-colors flex items-center gap-1">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-3 mb-3">
          ...
          {camerasPdv.length === 0 && (
            <div className="mt-2 text-sm text-amber-600 bg-amber-50 px-3 py-1.5 rounded-lg flex items-center gap-2">
              ...
              Configure as cameras por PDV clicando no botao "Cameras" acima para habilitar o video.
            </div>
          )}
        </div>
