...
export default function EmailMonitorTab() {
  ...
  const [whatsGroups, setWhatsGroups] = useState([]);
  ...
  const loadWhatsGroups = async () => {
    ...
    try {
      ...
      if (res.data?.success) {
        let groups = [];
        if (Array.isArray(res.data.data)) groups = res.data.data;
        else if (res.data.data?.groups) groups = res.data.data.groups;
        else if (res.data.data?.data) groups = res.data.data.data;
        else if (typeof res.data.data === 'object') groups = Object.values(res.data.data);
        console.log(`✅ ${groups.length} grupos carregados`);
        setWhatsGroups(groups);
        if (groups.length === 0) alert('Nenhum grupo encontrado');
      } else {
        alert('Falha ao carregar grupos: ' + (res.data?.error || 'resposta sem success'));
      }
    } catch (e) {
  ...
  const renderFiltersTab = () => (
    <div className="space-y-6">
      ...
      <div>
        ...
        {(() => {
          // Parse filtros: suporte a formato antigo (string[]) e novo (object[])
          let filters = [];
          try {
            const raw = config.subject_filter || '';
            if (raw.startsWith('[')) {
              const parsed = JSON.parse(raw);
              filters = parsed.map((f, i) => {
                if (typeof f === 'string') return { filter: f, type: i === 0 ? 'dvr' : 'custom', group: '', groupName: '', message: '' };
                return { filter: f.filter || '', type: f.type || (i === 0 ? 'dvr' : 'custom'), group: f.group || '', groupName: f.groupName || '', message: f.message || '' };
              });
            } else if (raw) {
              filters = [{ filter: raw, type: 'dvr', group: '', groupName: '', message: '' }];
            }
          } catch { filters = config.subject_filter ? [{ filter: config.subject_filter, type: 'dvr', group: '', groupName: '', message: '' }] : []; }
          if (filters.length === 0) filters = [{ filter: '', type: 'dvr', group: '', groupName: '', message: '' }];
          ...
          return (
            <div className="space-y-3">
              {filters.map((f, i) => (
                <div key={i} className={`p-3 rounded-lg border ${i === 0 ? 'border-blue-200 bg-blue-50/30' : 'border-gray-200 bg-gray-50/30'}`}>
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`text-xs font-bold px-2 py-0.5 rounded ${i === 0 ? 'bg-blue-100 text-blue-700' : 'bg-orange-100 text-orange-700'}`}>
                      {i === 0 ? '📹 DVR (padrao)' : `📧 Filtro ${i + 1}`}
                    </span>
                    {filters.length > 1 && i > 0 && (
                      <button type="button" onClick={() => { const nf = filters.filter((_, j) => j !== i); updateFilters(nf); }}
                        className="ml-auto p-1 text-red-400 hover:text-red-600 hover:bg-red-50 rounded" title="Remover filtro">
                  ...
                  <div className={`grid gap-2 ${i === 0 ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-3'}`}>
                    <div>
                      <input type="text" value={f.filter}
                        onChange={(e) => { const nf = [...filters]; nf[i] = { ...nf[i], filter: e.target.value }; updateFilters(nf); }}
                        placeholder="Texto do assunto (ex: ALERTA DVR)"
                      ...
                      <>
                        <div>
                          <div className="flex gap-1">
                            <select value={f.group}
                              onChange={(e) => {
                                const nf = [...filters];
                                const grp = whatsGroups.find(g => g.id === e.target.value);
                                nf[i] = { ...nf[i], group: e.target.value, groupName: grp?.subject || '' };
                                updateFilters(nf);
                              }}
                              className="flex-1 px-2 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-orange-500 focus:border-transparent">
                              <option value="">Selecione o grupo</option>
                              {whatsGroups.map(g => (
                                <option key={g.id} value={g.id}>{g.subject || g.id}</option>
                              ))}
                            </select>
                            <button type="button" onClick={loadWhatsGroups} disabled={loadingGroups}
                              className="px-2 py-1 text-xs bg-green-500 hover:bg-green-600 text-white rounded-lg disabled:opacity-50 whitespace-nowrap"
                              title="Carregar grupos do WhatsApp">
                              {loadingGroups ? '...' : '🔄'}
                            </button>
                          </div>
                          {f.groupName && <p className="text-[10px] text-green-600 mt-0.5">✅ {f.groupName}</p>}
                        </div>
                        <div>
                          <textarea value={f.message}
                            onChange={(e) => { const nf = [...filters]; nf[i] = { ...nf[i], message: e.target.value }; updateFilters(nf); }}
                            placeholder={"Mensagem a enviar\nUse Enter pra quebrar linha\nEmojis funcionam 🎉"}
                            rows={2}
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-orange-500 focus:border-transparent resize-y" />
                          <button type="button"
                            disabled={!f.group || !f.message?.trim()}
                            onClick={async () => {
                              try {
                                await api.post('/whatsapp/test-group', { groupId: f.group, message: f.message });
                                alert('✅ Mensagem enviada com sucesso!');
                              } catch (e) {
                                alert('❌ Erro ao enviar: ' + (e?.response?.data?.error || e.message));
                              }
                            }}
                            className="mt-1 px-3 py-1 text-xs bg-purple-500 hover:bg-purple-600 text-white rounded-lg disabled:opacity-40 flex items-center gap-1"
                            title="Envia a mensagem de teste pro grupo selecionado">
                            📤 Testar Envio
                          </button>
                        </div>
                      </>
                  ...
                  {i === 0 && <p className="text-[10px] text-blue-500 mt-1">Primeiro filtro usa comportamento DVR padrao (extrai PDF + envia pro grupo configurado)</p>}
                </div>
              ))}
              <button type="button"
                onClick={() => updateFilters([...filters, { filter: '', type: 'custom', group: '', groupName: '', message: '' }])}
        ...
        <p className="mt-1 text-xs text-gray-500">
          Emails que contenham QUALQUER um desses textos no assunto serão processados. Primeiro filtro = DVR. Demais = mensagem customizada pro grupo escolhido.
        </p>
      </div>
