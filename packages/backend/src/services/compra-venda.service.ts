...
export class CompraVendaService {
   ...
   */
  private static buildTipoNfFilter(tipoNotaFiscal?: CompraVendaFilters['tipoNotaFiscal'], colCfop = 'CFOP'): string {
    ...
    if (!tipoNotaFiscal) {
      return `AND TRIM(ni.${colCfop}) IN ('${cfopCompras.join("','")}')`;
    }
    ...
    if (outras && !compras && !bonificacao) {
      ...
      return `AND TRIM(ni.${colCfop}) NOT IN ('${cfopsExcluir.join("','")}')`;
    }
    ...
    if (cfopsIncluidos.length > 0) {
      return `AND TRIM(ni.${colCfop}) IN ('${cfopsIncluidos.join("','")}')`;
    }
   ...
   */
  private static buildTipoVendaFilter(tipoVenda?: CompraVendaFilters['tipoVenda'], colTipoSaida = 'TIPO_SAIDA'): string {
    ...
    if (tiposIncluidos.length > 0 && tiposIncluidos.length < 4) {
      return `AND pv.${colTipoSaida} IN (${tiposIncluidos.join(',')})`;
    }
  ...
  static async getCompraVendaPorSecao(filters: CompraVendaFilters): Promise<CompraVendaData[]> {
    ...
    const colValCustoRepPdv = await MappingService.getColumnFromTable('TAB_PRODUTO_PDV', 'valor_custo_reposicao');
    const colDtaSaida = await MappingService.getColumnFromTable('TAB_PRODUTO_PDV', 'data_venda');
    let colValImpostoDebito = 'VAL_IMPOSTO_DEBITO';
    try { const v = await MappingService.getColumnFromTable('TAB_PRODUTO_PDV', 'valor_imposto_debito'); if (v) colValImpostoDebito = v; } catch {}
    let colValImpostoCredito = 'VAL_IMPOSTO_CREDITO';
    try { const v = await MappingService.getColumnFromTable('TAB_PRODUTO_PDV', 'valor_imposto_credito'); if (v) colValImpostoCredito = v; } catch {}
    let colTipoSaida = 'TIPO_SAIDA';
    try { const v = await MappingService.getColumnFromTable('TAB_PRODUTO_PDV', 'tipo_saida'); if (v) colTipoSaida = v; } catch {}
    // Colunas NF Item extras
    let colQtdTotalNf = 'QTD_TOTAL';
    try { const v = await MappingService.getColumnFromTable('TAB_NF_ITEM', 'quantidade_total'); if (v) colQtdTotalNf = v; } catch {}
    let colCfop = 'CFOP';
    try { const v = await MappingService.getColumnFromTable('TAB_NF_ITEM', 'cfop'); if (v) colCfop = v; } catch {}
    // Colunas Decomposição
    let colQtdDecomp = 'QTD_DECOMP';
    try { const v = await MappingService.getColumnFromTable('TAB_PRODUTO_DECOMPOSICAO', 'quantidade_decomposicao'); if (v) colQtdDecomp = v; } catch {}
    let colCodProdutoDecom = 'COD_PRODUTO_DECOM';
    try { const v = await MappingService.getColumnFromTable('TAB_PRODUTO_DECOMPOSICAO', 'codigo_produto_decomposicao'); if (v) colCodProdutoDecom = v; } catch {}
    // Colunas Produção
    let colQtdProducao = 'QTD_PRODUCAO';
    try { const v = await MappingService.getColumnFromTable('TAB_PRODUTO_PRODUCAO', 'quantidade_producao'); if (v) colQtdProducao = v; } catch {}
    let colCodProdutoProducao = 'COD_PRODUTO_PRODUCAO';
    try { const v = await MappingService.getColumnFromTable('TAB_PRODUTO_PRODUCAO', 'codigo_produto_producao'); if (v) colCodProdutoProducao = v; } catch {}
    let colCodProdutoFinal = 'COD_PRODUTO';
    try { const v = await MappingService.getColumnFromTable('TAB_PRODUTO_PRODUCAO', 'codigo_produto'); if (v) colCodProdutoFinal = v; } catch {}
    // Colunas Associação (Produto Loja)
    let colCodAssociado = 'COD_ASSOCIADO';
    try { const v = await MappingService.getColumnFromTable('TAB_PRODUTO_LOJA', 'codigo_associado'); if (v) colCodAssociado = v; } catch {}
    // Embalagem
    let colQtdEmbalagemVenda = 'QTD_EMBALAGEM_VENDA';
    try { const v = await MappingService.getColumnFromTable('TAB_PRODUTO', 'qtd_embalagem_venda'); if (v) colQtdEmbalagemVenda = v; } catch {}
    ...
    // Construir filtros
    const tipoNfFilter = this.buildTipoNfFilter(tipoNotaFiscal, colCfop);
    const tipoVendaFilter = this.buildTipoVendaFilter(tipoVenda, colTipoSaida);
    ...
    // Query otimizada com CTEs - escaneia NF/NF_ITEM e PRODUTO_PDV apenas 1 vez
    // Antes: até 7 scans de NF+NF_ITEM e 5 scans de PRODUTO_PDV quando empréstimos ativos
    const needsEmprestimos = calcDecomposicao || calcProducao || calcAssociacao;
    const materializeHint = needsEmprestimos ? '/*+ MATERIALIZE */' : '';

    const sql = `
      WITH
      compras_raw AS (
        SELECT ${materializeHint} ni.${colCodItem} as COD_ITEM, nf.${colCodLoja} as COD_LOJA,
               SUM(ni.${colValTotalItem}) as VAL_TOTAL, SUM(ni.${colQtdTotalNf}) as QTD_TOTAL
        FROM ${tabNf} nf
      ...
      vendas_raw AS (
        SELECT ${materializeHint} pv.${colCodProduto} as COD_PRODUTO, pv.${colCodLoja} as COD_LOJA,
               SUM(pv.${colQtdTotalProduto}) as QTD_VENDIDA,
               SUM(pv.${colValTotalProduto}) as VALOR_VENDAS,
               SUM(pv.${colValCustoRepPdv} * pv.${colQtdTotalProduto}) as CUSTO_VENDA,
               SUM(NVL(pv.${colValImpostoDebito}, 0)) as TOTAL_IMPOSTO,
               SUM(NVL(pv.${colValImpostoCredito}, 0)) as TOTAL_IMPOSTO_CREDITO
      ...
      FROM ${tabSecao} sec
      -- Compras por seção (derivado do CTE compras_raw)
      LEFT JOIN (
        SELECT p.${colCodSecaoProd} as COD_SECAO, cr.COD_LOJA,
               SUM(cr.QTD_TOTAL) as QTD_COMPRA, SUM(cr.VAL_TOTAL) as VALOR_COMPRAS
        FROM compras_raw cr
        JOIN ${tabProduto} p ON cr.COD_ITEM = p.${colCodProduto}
        WHERE 1=1 ${produtoFilters}
        GROUP BY p.${colCodSecaoProd}, cr.COD_LOJA
      ) c ON sec.${colCodSecao} = c.COD_SECAO
      -- Vendas por seção (derivado do CTE vendas_raw)
      LEFT JOIN (
        SELECT p.${colCodSecaoProd} as COD_SECAO, vr.COD_LOJA,
               SUM(vr.QTD_VENDIDA) as QTD_VENDA,
               SUM(vr.VALOR_VENDAS) as VALOR_VENDAS,
               SUM(vr.CUSTO_VENDA) as CUSTO_VENDA,
               SUM(vr.TOTAL_IMPOSTO) as TOTAL_IMPOSTO,
               SUM(vr.TOTAL_IMPOSTO_CREDITO) as TOTAL_IMPOSTO_CREDITO
        FROM vendas_raw vr
        JOIN ${tabProduto} p ON vr.COD_PRODUTO = p.${colCodProduto}
        WHERE 1=1 ${produtoFilters}
        GROUP BY p.${colCodSecaoProd}, vr.COD_LOJA
      ...
      ${calcDecomposicao ? `
      -- EMPRESTEI (DECOMPOSIÇÃO): Produtos PAI que emprestam custo para filhos
      LEFT JOIN (
        SELECT p.${colCodSecaoProd} as COD_SECAO, cr.COD_LOJA, SUM(cr.VAL_TOTAL) as VALOR_EMPRESTEI
        FROM compras_raw cr
        JOIN ${tabProduto} p ON cr.COD_ITEM = p.${colCodProduto}
        WHERE EXISTS (SELECT 1 FROM ${tabProdutoDecomposicao} d WHERE d.${colCodProduto} = cr.COD_ITEM)
        GROUP BY p.${colCodSecaoProd}, cr.COD_LOJA
      ...
      LEFT JOIN (
        SELECT p_filho.${colCodSecaoProd} as COD_SECAO, cr.COD_LOJA,
               SUM(cr.VAL_TOTAL * d.${colQtdDecomp} / 100) as VALOR_EMPRESTADO
        FROM compras_raw cr
        JOIN ${tabProdutoDecomposicao} d ON cr.COD_ITEM = d.${colCodProduto}
        JOIN ${tabProduto} p_filho ON d.${colCodProdutoDecom} = p_filho.${colCodProduto}
        GROUP BY p_filho.${colCodSecaoProd}, cr.COD_LOJA
      ...
      LEFT JOIN (
        SELECT p_insumo.${colCodSecaoProd} as COD_SECAO, cr.COD_LOJA,
               SUM(vr.QTD_VENDIDA * pp.${colQtdProducao} * (cr.VAL_TOTAL / NULLIF(cr.QTD_TOTAL, 0))) as VALOR_EMPRESTEI
        FROM ${tabProdutoProducao} pp
        JOIN ${tabProduto} p_insumo ON pp.${colCodProdutoProducao} = p_insumo.${colCodProduto}
        JOIN compras_raw cr ON pp.${colCodProdutoProducao} = cr.COD_ITEM
        JOIN vendas_raw vr ON pp.${colCodProdutoFinal} = vr.COD_PRODUTO AND cr.COD_LOJA = vr.COD_LOJA
        GROUP BY p_insumo.${colCodSecaoProd}, cr.COD_LOJA
      ...
      -- EMPRESTADO (PRODUÇÃO): Produtos FINAIS recebem custo dos insumos
      LEFT JOIN (
        SELECT p_final.${colCodSecaoProd} as COD_SECAO, vr.COD_LOJA,
               SUM(vr.QTD_VENDIDA * pp.${colQtdProducao} * (cr.VAL_TOTAL / NULLIF(cr.QTD_TOTAL, 0))) as VALOR_EMPRESTADO
        FROM ${tabProdutoProducao} pp
        JOIN ${tabProduto} p_final ON pp.${colCodProdutoFinal} = p_final.${colCodProduto}
        JOIN vendas_raw vr ON pp.${colCodProdutoFinal} = vr.COD_PRODUTO
        LEFT JOIN compras_raw cr ON pp.${colCodProdutoProducao} = cr.COD_ITEM AND vr.COD_LOJA = cr.COD_LOJA
        GROUP BY p_final.${colCodSecaoProd}, vr.COD_LOJA
      ...
      LEFT JOIN (
        SELECT p_base.${colCodSecaoProd} as COD_SECAO, cr.COD_LOJA,
               SUM(vr.QTD_VENDIDA * NVL(p_venda.${colQtdEmbalagemVenda}, 1) * (cr.VAL_TOTAL / NULLIF(cr.QTD_TOTAL, 0))) as VALOR_EMPRESTEI
        FROM ${tabProdutoLoja} pl
        JOIN ${tabProduto} p_venda ON pl.${colCodProduto} = p_venda.${colCodProduto}
        JOIN ${tabProduto} p_base ON pl.${colCodAssociado} = p_base.${colCodProduto}
        JOIN compras_raw cr ON pl.${colCodAssociado} = cr.COD_ITEM
        JOIN vendas_raw vr ON pl.${colCodProduto} = vr.COD_PRODUTO AND cr.COD_LOJA = vr.COD_LOJA
        WHERE pl.${colCodAssociado} IS NOT NULL
        GROUP BY p_base.${colCodSecaoProd}, cr.COD_LOJA
      ...
      LEFT JOIN (
        SELECT p_venda.${colCodSecaoProd} as COD_SECAO, vr.COD_LOJA,
               SUM(vr.QTD_VENDIDA * NVL(p_venda.${colQtdEmbalagemVenda}, 1) * (cr.VAL_TOTAL / NULLIF(cr.QTD_TOTAL, 0))) as VALOR_EMPRESTADO
        FROM ${tabProdutoLoja} pl
        JOIN ${tabProduto} p_venda ON pl.${colCodProduto} = p_venda.${colCodProduto}
        JOIN vendas_raw vr ON pl.${colCodProduto} = vr.COD_PRODUTO
        LEFT JOIN compras_raw cr ON pl.${colCodAssociado} = cr.COD_ITEM AND vr.COD_LOJA = cr.COD_LOJA
        WHERE pl.${colCodAssociado} IS NOT NULL
        GROUP BY p_venda.${colCodSecaoProd}, vr.COD_LOJA
    ...
    console.log(`⏱️ Compra x Venda por Seção: ${Date.now() - t0}ms (${rows.length} rows, emprestimos=${needsEmprestimos})`);
  ...
  static async getCompraVendaPorGrupo(filters: CompraVendaFilters & { codSecao: number }): Promise<any[]> {
    ...
    // Resolver tabelas e colunas em paralelo (Promise.all para evitar awaits sequenciais)
    const schema = await MappingService.getSchema();
    const [
      tabGrupoName, tabNfName, tabNfItemName, tabProdutoName, tabProdutoPdvName,
      tabProdutoDecomposicaoName, tabProdutoProducaoName, tabProdutoLojaName, tabProdutoCompradorName,
      colCodGrupo, colDesGrupo, colCodSecao,
      colCodProduto, colCodSecaoProd, colCodGrupoProd,
      colNumNf, colSerieNf, colDtaEntrada, colCodParceiro, colTipoOperacao,
      colNumNfItem, colSerieNfItem, colCodParceiroItem, colCodItem, colValTotalItem, colCodLoja,
      colQtdTotalProduto, colValTotalProduto, colValCustoRepPdv, colDtaSaida,
      colValImpostoDebitoRaw, colValImpositoCreditoRaw, colTipoSaidaRaw,
      colQtdTotalNfRaw, colCfopRaw,
      colQtdDecompRaw, colCodProdutoDecomRaw,
      colQtdProducaoRaw, colCodProdutoProducaoRaw, colCodProdutoFinalRaw,
      colCodAssociadoRaw, colQtdEmbalagemVendaRaw
    ] = await Promise.all([
      ...
      MappingService.getRealTableName('TAB_PRODUTO_PRODUCAO'),
      MappingService.getRealTableName('TAB_PRODUTO_LOJA'),
      MappingService.getRealTableName('TAB_PRODUTO_COMPRADOR'),
      ...
      MappingService.getColumnFromTable('TAB_NF', 'numero_nf'),
      MappingService.getColumnFromTable('TAB_NF', 'serie_nf'),
      MappingService.getColumnFromTable('TAB_NF', 'data_entrada'),
      MappingService.getColumnFromTable('TAB_NF', 'codigo_parceiro'),
      MappingService.getColumnFromTable('TAB_NF', 'tipo_operacao'),
      MappingService.getColumnFromTable('TAB_NF_ITEM', 'numero_nf'),
      MappingService.getColumnFromTable('TAB_NF_ITEM', 'serie_nf'),
      MappingService.getColumnFromTable('TAB_NF_ITEM', 'codigo_parceiro'),
      MappingService.getColumnFromTable('TAB_NF_ITEM', 'codigo_item'),
      MappingService.getColumnFromTable('TAB_NF_ITEM', 'valor_total'),
      MappingService.getColumnFromTable('TAB_LOJA', 'codigo_loja'),
      ...
      MappingService.getColumnFromTable('TAB_PRODUTO_PDV', 'valor_custo_reposicao'),
      MappingService.getColumnFromTable('TAB_PRODUTO_PDV', 'data_venda'),
      MappingService.getColumnFromTable('TAB_PRODUTO_PDV', 'valor_imposto_debito').catch(() => null),
      MappingService.getColumnFromTable('TAB_PRODUTO_PDV', 'valor_imposto_credito').catch(() => null),
      MappingService.getColumnFromTable('TAB_PRODUTO_PDV', 'tipo_saida').catch(() => null),
      MappingService.getColumnFromTable('TAB_NF_ITEM', 'quantidade_total').catch(() => null),
      MappingService.getColumnFromTable('TAB_NF_ITEM', 'cfop').catch(() => null),
      MappingService.getColumnFromTable('TAB_PRODUTO_DECOMPOSICAO', 'quantidade_decomposicao').catch(() => null),
      MappingService.getColumnFromTable('TAB_PRODUTO_DECOMPOSICAO', 'codigo_produto_decomposicao').catch(() => null),
      MappingService.getColumnFromTable('TAB_PRODUTO_PRODUCAO', 'quantidade_producao').catch(() => null),
      MappingService.getColumnFromTable('TAB_PRODUTO_PRODUCAO', 'codigo_produto_producao').catch(() => null),
      MappingService.getColumnFromTable('TAB_PRODUTO_PRODUCAO', 'codigo_produto').catch(() => null),
      MappingService.getColumnFromTable('TAB_PRODUTO_LOJA', 'codigo_associado').catch(() => null),
      MappingService.getColumnFromTable('TAB_PRODUTO', 'qtd_embalagem_venda').catch(() => null),
    ]);

    const tabGrupo = `${schema}.${tabGrupoName}`;
    const tabNf = `${schema}.${tabNfName}`;
    const tabNfItem = `${schema}.${tabNfItemName}`;
    const tabProduto = `${schema}.${tabProdutoName}`;
    const tabProdutoPdv = `${schema}.${tabProdutoPdvName}`;
    const tabProdutoDecomposicao = `${schema}.${tabProdutoDecomposicaoName}`;
    const tabProdutoProducao = `${schema}.${tabProdutoProducaoName}`;
    const tabProdutoLoja = `${schema}.${tabProdutoLojaName}`;
    const tabProdutoComprador = `${schema}.${tabProdutoCompradorName}`;
    const colValImpostoDebito = colValImpostoDebitoRaw || 'VAL_IMPOSTO_DEBITO';
    const colValImpostoCredito = colValImpositoCreditoRaw || 'VAL_IMPOSTO_CREDITO';
    const colTipoSaida = colTipoSaidaRaw || 'TIPO_SAIDA';
    const colQtdTotalNf = colQtdTotalNfRaw || 'QTD_TOTAL';
    const colCfop = colCfopRaw || 'CFOP';
    const colQtdDecomp = colQtdDecompRaw || 'QTD_DECOMP';
    const colCodProdutoDecom = colCodProdutoDecomRaw || 'COD_PRODUTO_DECOM';
    const colQtdProducao = colQtdProducaoRaw || 'QTD_PRODUCAO';
    const colCodProdutoProducao = colCodProdutoProducaoRaw || 'COD_PRODUTO_PRODUCAO';
    const colCodProdutoFinal = colCodProdutoFinalRaw || 'COD_PRODUTO';
    const colCodAssociado = colCodAssociadoRaw || 'COD_ASSOCIADO';
    const colQtdEmbalagemVenda = colQtdEmbalagemVendaRaw || 'QTD_EMBALAGEM_VENDA';
    ...
    const sql = `
      ...
      LEFT JOIN (
        SELECT
          ...
          SUM(ni.${colQtdTotalNf}) as QTD_COMPRA,
      ...
      LEFT JOIN (
        SELECT
          ...
          SUM(pv.${colQtdTotalProduto}) as QTD_VENDA,
          SUM(pv.${colValTotalProduto}) as VALOR_VENDAS,
          SUM(pv.${colValCustoRepPdv} * pv.${colQtdTotalProduto}) as CUSTO_VENDA,
          SUM(NVL(pv.${colValImpostoDebito}, 0)) as TOTAL_IMPOSTO,
          SUM(NVL(pv.${colValImpostoCredito}, 0)) as TOTAL_IMPOSTO_CREDITO
      ...
      LEFT JOIN (
        SELECT
          ...
          nf.${colCodLoja} as COD_LOJA,
          SUM(ni.${colValTotalItem} * d.${colQtdDecomp} / 100) as VALOR_EMPRESTADO
        FROM ${tabNf} nf
        JOIN ${tabNfItem} ni ON nf.${colNumNf} = ni.${colNumNfItem}
          AND nf.${colSerieNf} = ni.${colSerieNfItem}
          AND nf.${colCodParceiro} = ni.${colCodParceiroItem}
      ...
      LEFT JOIN (
        SELECT
          ...
          compras.COD_LOJA,
          SUM(
            vendas.QTD_VENDIDA * pp.${colQtdProducao} *
            (compras.VAL_TOTAL / NULLIF(compras.QTD_TOTAL, 0))
          ) as VALOR_EMPRESTEI
        FROM ${tabProdutoProducao} pp
        JOIN ${tabProduto} p_insumo ON pp.${colCodProdutoProducao} = p_insumo.${colCodProduto}
        JOIN (
          SELECT ni.${colCodItem} as COD_ITEM, nf.${colCodLoja} as COD_LOJA, SUM(ni.${colValTotalItem}) as VAL_TOTAL, SUM(ni.${colQtdTotalNf}) as QTD_TOTAL
          FROM ${tabNf} nf
          ...
          ${lojaFilterCompras}
          GROUP BY ni.${colCodItem}, nf.${colCodLoja}
        ) compras ON pp.${colCodProdutoProducao} = compras.COD_ITEM
        JOIN (
          SELECT pv.${colCodProduto} as COD_PRODUTO, pv.${colCodLoja} as COD_LOJA, SUM(pv.${colQtdTotalProduto}) as QTD_VENDIDA
          FROM ${tabProdutoPdv} pv
          WHERE pv.${colDtaSaida} BETWEEN TO_DATE(:dataInicio, 'DD/MM/YYYY') AND TO_DATE(:dataFim, 'DD/MM/YYYY')
          ${tipoVendaFilter}
          ${lojaFilterVendas}
          GROUP BY pv.${colCodProduto}, pv.${colCodLoja}
        ) vendas ON pp.${colCodProdutoFinal} = vendas.COD_PRODUTO AND compras.COD_LOJA = vendas.COD_LOJA
      ...
      LEFT JOIN (
        ...
        JOIN (
          ...
          ${lojaFilterVendas}
          GROUP BY pv.${colCodProduto}, pv.${colCodLoja}
        ) vendas ON pp.${colCodProdutoFinal} = vendas.COD_PRODUTO
        LEFT JOIN (
          SELECT ni.${colCodItem} as COD_ITEM, nf.${colCodLoja} as COD_LOJA, SUM(ni.${colValTotalItem}) as VAL_TOTAL, SUM(ni.${colQtdTotalNf}) as QTD_TOTAL
          FROM ${tabNf} nf
          JOIN ${tabNfItem} ni ON nf.${colNumNf} = ni.${colNumNfItem} AND nf.${colSerieNf} = ni.${colSerieNfItem} AND nf.${colCodParceiro} = ni.${colCodParceiroItem}
          ...
          ${lojaFilterCompras}
          GROUP BY ni.${colCodItem}, nf.${colCodLoja}
        ) compras ON pp.${colCodProdutoProducao} = compras.COD_ITEM AND vendas.COD_LOJA = compras.COD_LOJA
      ...
      LEFT JOIN (
        SELECT
          ...
          compras.COD_LOJA,
          SUM(
            vendas.QTD_VENDIDA * NVL(p_venda.${colQtdEmbalagemVenda}, 1) *
            (compras.VAL_TOTAL / NULLIF(compras.QTD_TOTAL, 0))
          ) as VALOR_EMPRESTEI
        FROM ${tabProdutoLoja} pl
        JOIN ${tabProduto} p_venda ON pl.${colCodProduto} = p_venda.${colCodProduto}
        ...
        JOIN (
          ...
          ${lojaFilterCompras}
          GROUP BY ni.${colCodItem}, nf.${colCodLoja}
        ) compras ON pl.${colCodAssociado} = compras.COD_ITEM
        JOIN (
          SELECT pv.${colCodProduto} as COD_PRODUTO, pv.${colCodLoja} as COD_LOJA, SUM(pv.${colQtdTotalProduto}) as QTD_VENDIDA
          FROM ${tabProdutoPdv} pv
          WHERE pv.${colDtaSaida} BETWEEN TO_DATE(:dataInicio, 'DD/MM/YYYY') AND TO_DATE(:dataFim, 'DD/MM/YYYY')
        ...
        WHERE pl.${colCodAssociado} IS NOT NULL AND p_base.${colCodSecaoProd} = :codSecao
      ...
      LEFT JOIN (
        ...
        LEFT JOIN (
          ...
          ${lojaFilterCompras}
          GROUP BY ni.${colCodItem}, nf.${colCodLoja}
        ) compras ON pl.${colCodAssociado} = compras.COD_ITEM AND vendas.COD_LOJA = compras.COD_LOJA
        WHERE pl.${colCodAssociado} IS NOT NULL AND p_venda.${colCodSecaoProd} = :codSecao
  ...
  static async getCompraVendaPorSubGrupo(filters: CompraVendaFilters & { codSecao: number; codGrupo: number }): Promise<any[]> {
    ...
    const schema = await MappingService.getSchema();
    const [
      tabSubgrupoName, tabNfName, tabNfItemName, tabProdutoName, tabProdutoPdvName,
      tabProdutoDecomposicaoName, tabProdutoProducaoName, tabProdutoLojaName, tabProdutoCompradorName,
      colCodSubGrupo, colDesSubGrupo, colCodSecao, colCodGrupo,
      colCodProduto, colCodSecaoProd, colCodGrupoProd, colCodSubGrupoProd,
      colNumNf, colSerieNf, colDtaEntrada, colCodParceiro, colTipoOperacao,
      colNumNfItem, colSerieNfItem, colCodParceiroItem, colCodItem, colValTotalItem, colCodLoja,
      colQtdTotalProduto, colValTotalProduto, colValCustoRepPdv, colDtaSaida,
      colValImpostoDebitoRaw, colValImpositoCreditoRaw, colTipoSaidaRaw,
    ...
    const tabSubgrupo = `${schema}.${tabSubgrupoName}`;
    const tabNf = `${schema}.${tabNfName}`;
    const tabNfItem = `${schema}.${tabNfItemName}`;
    const tabProduto = `${schema}.${tabProdutoName}`;
    const tabProdutoPdv = `${schema}.${tabProdutoPdvName}`;
    ...
    const sql = `
      ...
      LEFT JOIN (
        ...
        WHERE pl.${colCodAssociado} IS NOT NULL AND p_base.${colCodSecaoProd} = :codSecao AND p_base.${colCodGrupoProd} = :codGrupo
      ...
      LEFT JOIN (
        ...
        WHERE pl.${colCodAssociado} IS NOT NULL AND p_venda.${colCodSecaoProd} = :codSecao AND p_venda.${colCodGrupoProd} = :codGrupo
  ...
  static async getCompraVendaPorItem(filters: CompraVendaFilters & { codSecao: number; codGrupo: number; codSubGrupo: number }): Promise<any[]> {
    ...
    // Resolver tabelas e colunas em paralelo (Promise.all para evitar 43 awaits sequenciais)
    const schema = await MappingService.getSchema();
    const [
      tabProdutoName, tabNfName, tabNfItemName, tabProdutoPdvName,
      tabProdutoDecomposicaoName, tabProdutoProducaoName, tabProdutoLojaName, tabProdutoCompradorName,
      colCodProduto, colDesProduto, colCodSecaoProd, colCodGrupoProd, colCodSubGrupoProd,
      colNumNf, colSerieNf, colDtaEntrada, colCodParceiro, colTipoOperacao,
      colNumNfItem, colSerieNfItem, colCodParceiroItem, colCodItem, colValTotalItem,
      colCodLoja,
      colQtdTotalProduto, colValTotalProduto, colValCustoRepPdv, colDtaSaida,
      colValImpostoDebitoRaw, colValImpositoCreditoRaw, colTipoSaidaRaw,
      colQtdTotalNfRaw, colCfopRaw,
    ...
    const sql = `
      SELECT
        ...
        + ${calcProducao ? 'NVL(emp_final.VALOR_EMPRESTADO, 0)' : '0'}
        + ${calcAssociacao ? 'NVL(emp_assoc_filho.VALOR_EMPRESTADO, 0)' : '0'} as EMPRESTADO,
        NVL(est.${colEstoqueAtual}, 0) as ESTOQUE_ATUAL,
        NVL(est.${colCobertura}, 0) as DIAS_COBERTURA,
        CASE WHEN EXISTS (
          SELECT 1 FROM ${tabProdutoProducao} pp_rec WHERE pp_rec.${colCodProdutoFinal} = p.${colCodProduto}
      ...
      LEFT JOIN (
        SELECT
          d.${colCodProdutoDecom} as COD_PRODUTO,
          nf.${colCodLoja} as COD_LOJA,
          SUM(ni.${colValTotalItem} * d.${colQtdDecomp} / 100) as VALOR_EMPRESTADO
        FROM ${tabNf} nf
        JOIN ${tabNfItem} ni ON nf.${colNumNf} = ni.${colNumNfItem}
        ...
        ${tipoNfFilter}
        ${lojaFilterCompras}
        GROUP BY d.${colCodProdutoDecom}, nf.${colCodLoja}
      ...
      LEFT JOIN (
        SELECT
          pp.${colCodProdutoProducao} as COD_PRODUTO,
          compras.COD_LOJA,
          SUM(
            vendas.QTD_VENDIDA * pp.${colQtdProducao} *
            (compras.VAL_TOTAL / NULLIF(compras.QTD_TOTAL, 0))
        ...
        GROUP BY pp.${colCodProdutoProducao}, compras.COD_LOJA
      ...
      LEFT JOIN (
        ...
        GROUP BY pp.${colCodProdutoFinal}, vendas.COD_LOJA
      ...
      LEFT JOIN (
        SELECT
          pl.${colCodAssociado} as COD_PRODUTO,
          compras.COD_LOJA,
          SUM(
            vendas.QTD_VENDIDA * NVL(p_venda.${colQtdEmbalagemVenda}, 1) *
            (compras.VAL_TOTAL / NULLIF(compras.QTD_TOTAL, 0))
        ...
        WHERE pl.${colCodAssociado} IS NOT NULL
        GROUP BY pl.${colCodAssociado}, compras.COD_LOJA
  ...
  static async getDetalheEmprestimo(filters: CompraVendaFilters & {
    ...
    const colDtaSaida = await MappingService.getColumnFromTable('TAB_PRODUTO_PDV', 'data_venda');
    let colTipoSaida = 'TIPO_SAIDA';
    try { const v2 = await MappingService.getColumnFromTable('TAB_PRODUTO_PDV', 'tipo_saida'); if (v2) colTipoSaida = v2; } catch {}
    let colQtdTotalNf = 'QTD_TOTAL';
    try { const v = await MappingService.getColumnFromTable('TAB_NF_ITEM', 'quantidade_total'); if (v) colQtdTotalNf = v; } catch {}
    let colCfop = 'CFOP';
    try { const v2 = await MappingService.getColumnFromTable('TAB_NF_ITEM', 'cfop'); if (v2) colCfop = v2; } catch {}
    let colQtdDecomp = 'QTD_DECOMP';
    try { const v2 = await MappingService.getColumnFromTable('TAB_PRODUTO_DECOMPOSICAO', 'quantidade_decomposicao'); if (v2) colQtdDecomp = v2; } catch {}
    let colCodProdutoDecom = 'COD_PRODUTO_DECOM';
    try { const v2 = await MappingService.getColumnFromTable('TAB_PRODUTO_DECOMPOSICAO', 'codigo_produto_decomposicao'); if (v2) colCodProdutoDecom = v2; } catch {}
    let colQtdProducao = 'QTD_PRODUCAO';
    try { const v2 = await MappingService.getColumnFromTable('TAB_PRODUTO_PRODUCAO', 'quantidade_producao'); if (v2) colQtdProducao = v2; } catch {}
    let colCodProdutoProducao = 'COD_PRODUTO_PRODUCAO';
    try { const v2 = await MappingService.getColumnFromTable('TAB_PRODUTO_PRODUCAO', 'codigo_produto_producao'); if (v2) colCodProdutoProducao = v2; } catch {}
    let colCodProdutoFinal = 'COD_PRODUTO';
    try { const v2 = await MappingService.getColumnFromTable('TAB_PRODUTO_PRODUCAO', 'codigo_produto'); if (v2) colCodProdutoFinal = v2; } catch {}
    let colCodAssociado = 'COD_ASSOCIADO';
    try { const v2 = await MappingService.getColumnFromTable('TAB_PRODUTO_LOJA', 'codigo_associado'); if (v2) colCodAssociado = v2; } catch {}
    let colQtdEmbalagemVenda = 'QTD_EMBALAGEM_VENDA';
    try { const v2 = await MappingService.getColumnFromTable('TAB_PRODUTO', 'qtd_embalagem_venda'); if (v2) colQtdEmbalagemVenda = v2; } catch {}
    let colCodLojaPdv = 'COD_LOJA';
    try { const v2 = await MappingService.getColumnFromTable('TAB_PRODUTO_PDV', 'codigo_loja'); if (v2) colCodLojaPdv = v2; } catch {}
    ...
    if (codLoja) {
      ...
      lojaFilterVendas = ` AND pv.${colCodLojaPdv} = :codLoja`;
      params.codLoja = codLoja;
    }
    ...
    if (tipo === 'emprestei') {
      ...
      const sqlDecomp = `
        SELECT
          ...
          d.${colQtdDecomp} as PERCENTUAL,
          NVL(compras.VAL_TOTAL, 0) * d.${colQtdDecomp} / 100 as VALOR
      ...
      const sqlDecomp = `
        ...
        JOIN ${tabProdutoDecomposicao} d ON p_filho.${colCodProduto} = d.${colCodProdutoDecom}
        JOIN ${tabProduto} p_pai ON d.${colCodProduto} = p_pai.${colCodProduto}
        LEFT JOIN ${tabSecao} sec_pai ON p_pai.${colCodSecaoProd} = sec_pai.${colCodSecao}
        LEFT JOIN ${tabGrupo} grp_pai ON p_pai.${colCodSecaoProd} = grp_pai.${colCodGrupoGrp} AND p_pai.${colCodGrupoProd} = grp_pai.${colCodGrupo}
        LEFT JOIN ${tabSubgrupo} sgp_pai ON p_pai.${colCodSecaoProd} = sgp_pai.${colCodSecaoSG} AND p_pai.${colCodGrupoProd} = sgp_pai.${colCodGrupoSG} AND p_pai.${colCodSubGrupoProd} = sgp_pai.${colCodSubGrupo}
        LEFT JOIN ${tabSecao} sec_filho ON p_filho.${colCodSecaoProd} = sec_filho.${colCodSecao}
    ...
    if (tipo === 'emprestei') {
      ...
      const sqlProd = `
        SELECT
          ...
          pp.${colQtdProducao} as QTD_RECEITA,
          NVL(vendas.QTD_VENDIDA, 0) * pp.${colQtdProducao} * (NVL(compras.VAL_TOTAL, 0) / NULLIF(compras.QTD_TOTAL, 0)) as VALOR
        FROM ${tabProdutoProducao} pp
        JOIN ${tabProduto} p_insumo ON pp.${colCodProdutoProducao} = p_insumo.${colCodProduto}
        JOIN ${tabProduto} p_final ON pp.${colCodProdutoFinal} = p_final.${colCodProduto}
        ...
        LEFT JOIN (
          SELECT ni.${colCodItem} as COD_ITEM, SUM(ni.${colValTotalItem}) as VAL_TOTAL, SUM(ni.${colQtdTotalNf}) as QTD_TOTAL
          FROM ${tabNf} nf
          JOIN ${tabNfItem} ni ON nf.${colNumNf} = ni.${colNumNfItem}
            AND nf.${colSerieNf} = ni.${colSerieNfItem}
            AND nf.${colCodParceiro} = ni.${colCodParceiroItem}
        ...
        LEFT JOIN (
          SELECT pv.COD_PRODUTO, SUM(pv.${colQtdTotalProduto}) as QTD_VENDIDA
          FROM ${tabProdutoPdv} pv
          WHERE pv.${colDtaSaida} BETWEEN TO_DATE(:dataInicio, 'DD/MM/YYYY') AND TO_DATE(:dataFim, 'DD/MM/YYYY')
          ${tipoVendaFilter}
          ${lojaFilterVendas}
  ...
  static async getReceitaProduto(codProduto: number): Promise<any> {
    ...
    const insumos = await OracleService.query<any>(`
      SELECT
        pp.${colCodProdutoProducao} as COD_INSUMO,
        p.${colDesProduto} as INSUMO,
        pp.${colQtdProducao} as QTD_RECEITA
      FROM ${tabProdutoProducao} pp
      JOIN ${tabProduto} p ON pp.${colCodProdutoProducao} = p.${colCodProduto}
      WHERE pp.${colCodProdutoFinal} = :codProduto
      ORDER BY pp.${colQtdProducao} DESC
    `, { codProduto });
