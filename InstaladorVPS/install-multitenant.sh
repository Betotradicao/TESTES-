...
# NOTA: Usa temp file para compatibilidade com curl | bash
SQL_TMP=$(mktemp /tmp/sql_XXXXXX.sql)
cat > "$SQL_TMP" << 'EOSQL'
...
EOSQL
cat "$SQL_TMP" | docker exec -i ${CONTAINER_PREFIX}-postgres psql -U $POSTGRES_USER -d $POSTGRES_DB_NAME 2>/dev/null || true
rm -f "$SQL_TMP"
