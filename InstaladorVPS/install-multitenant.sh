...
docker exec -i ${CONTAINER_PREFIX}-postgres psql -U $POSTGRES_USER -d $POSTGRES_DB_NAME << LOGOEOF 2>/dev/null || true
...
# Executar (garante que curl | bash leia o script inteiro antes de rodar)
main "$@"
