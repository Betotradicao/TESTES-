...
## 📅 BUG #5: CALENDÁRIO DE PERDAS - PERÍODO PERSONALIZADO

### Descrição do Problema

Na página **Lançador de Perdas**, quando criava um lote com **período personalizado** (ex: 01/01 a 03/01), o calendário apresentava dois problemas:

#### Problema 1: Datas não ficavam verdes
- ❌ Quando selecionava período e criava lote, os dias não ficavam verdes
- ❌ Dias continuavam vermelhos mesmo após criar o registro

#### Problema 2: Apenas data única funcionava
- ✅ Lote de um único dia (ex: só dia 3): funcionava
- ❌ Lote de múltiplos dias (ex: dia 1 ao 3): apenas alguns dias ficavam verdes

### Causa Raiz

#### Bug 1: Parsing de Datas no Frontend (Commit 1d02056)

**Problema:** Datas vinham da API com timestamp completo, código concatenava `T12:00:00` criando string inválida.

```javascript
// ❌ ANTES (ERRADO):
const dataInicio = new Date(lote.dataInicioPeriodo + 'T12:00:00');
// Se dataInicioPeriodo = "2026-01-02T00:00:00.000Z"
// Resultado: "2026-01-02T00:00:00.000ZT12:00:00" = Invalid Date
```

**Arquivo:** `packages/frontend/src/pages/PerdasLancador.jsx` linhas 84-85, 100, 115-116, 128

#### Bug 2: Data de Importação Errada (Commit 75b3baa)

**Problema:** Backend usava data inicial do período como `dataImportacao` em vez da data atual.

```typescript
// ❌ ANTES (ERRADO):
const dataImportacao = dataInicioCustom ? new Date(dataInicioCustom + 'T12:00:00') : new Date();
// Criar lote HOJE (03/01) com período 01/01 a 03/01
// dataImportacao ficava: 01/01 (data do período) ❌
// Deveria ser: 03/01 (data atual) ✅
```

**Arquivo:** `packages/backend/src/services/loss.service.ts` linha 117

### Solução Implementada

#### Correção 1: Fix Parsing de Datas (Commit 1d02056)

Extrair apenas parte YYYY-MM-DD antes de adicionar horário:
...
// Garante formato correto: "2026-01-02" + "T12:00:00" = data válida
```

**Arquivos Modificados:**
- `packages/frontend/src/pages/PerdasLancador.jsx` (linhas 84-85, 100, 115-116, 128)
- Funções: `isDayInLotePeriodo()` e `getDayLotes()`
- Timeout aumentado de 2s para 3s para visualizar calendário atualizado

#### Correção 2: Fix Data de Importação (Commit 75b3baa)

Usar SEMPRE data atual como `dataImportacao`:
...
**Semântica Correta:**
- `dataImportacao`: quando o lote foi **criado** (sempre data atual)
- `dataInicioPeriodo`: **início** do período escolhido pelo usuário
- `dataFimPeriodo`: **fim** do período escolhido pelo usuário

### Correção dos Dados Existentes

Lotes antigos com `dataImportacao` errada foram corrigidos:

```sql
-- Corrigir lote com período 01/01 a 03/01 criado em 03/01
UPDATE losses
SET data_importacao = '2026-01-03'
WHERE nome_lote = 'Perdas 03-01-2026 13h24'
  AND data_inicio_periodo = '2026-01-01'
  AND data_fim_periodo = '2026-01-03';

-- Resultado: 4.390 registros corrigidos ✅
```

### Validação da Correção

**Criar lote com período 01/01 a 03/01:**

✅ `dataImportacao`: 03/01/2026 (data atual de criação)
✅ `dataInicioPeriodo`: 01/01/2026 (início do período)
✅ `dataFimPeriodo`: 03/01/2026 (fim do período)
✅ **Calendário: dias 1, 2 e 3 ficam VERDES** 🟢
✅ Parsing de datas funciona com timestamps e datas simples
✅ Timeout permite visualizar calendário antes de redirecionar

### Commits Relacionados

```bash
# 1. Fix parsing de datas no frontend
1d02056 - fix: Corrige calendário de perdas não destacando datas com lotes

# 2. Fix data de importação no backend
75b3baa - fix: Corrige data de importação de lotes de perdas com período personalizado
```

### Resultado Final

✅ **Calendário destaca corretamente todos os dias do período em verde**
✅ **Lotes de múltiplos dias funcionam perfeitamente**
✅ **Data de importação sempre reflete quando o lote foi criado**
✅ **Período define quais dias aparecem verdes no calendário**
...
### ✅ Calendário de Perdas
- [x] Parsing de datas corrigido no frontend
- [x] Data de importação corrigida no backend
- [x] Dados antigos corrigidos no banco
- [x] Calendário mostra dias do período em verde
- [x] Períodos de múltiplos dias funcionam
- [x] Frontend e backend deployed

### ✅ Documentação
- [x] Documentação atualizada
- [x] Script de correção criado
- [x] Instruções para novas instalações documentadas
- [x] Bug #5 documentado com causa raiz e solução
